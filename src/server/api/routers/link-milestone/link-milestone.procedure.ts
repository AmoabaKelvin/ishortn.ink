...
export const linkMilestoneRouter = createTRPCRouter({
  getByLinkId: workspaceProcedure.input(getLinkMilestonesSchema).query(async ({ ctx, input }) => {
    return getLinkMilestones(ctx, input);
  }),

  upsert: workspaceProcedure.input(upsertMilestonesSchema).mutation(async ({ ctx, input }) => {
