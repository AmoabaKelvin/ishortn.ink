  ...
  for (let idx = 0; idx < delaunay.triangles.length; idx += 3) {
    ...
    for (let y = Math.floor(minY); y <= maxY + 1; y++) {
      for (let x = Math.floor(minX); x <= maxX + 1; x++) {
        ...
        if (isPointInTriangle(x, y, p1!, p2!, p3!)) trianglePoints.push([x, y]);
      }
    }

    if (trianglePoints.length === 0) continue;

    const colorSum = trianglePoints.reduce(
      (acc, [x, y]) => {
        const index = (y! * width + x!) * 4;
        acc[0]! += data[index] ?? 0;
        acc[1]! += data[index + 1] ?? 0;
        acc[2]! += data[index + 2] ?? 0;
        acc[3]! += data[index + 3] ?? 0;
        return acc;
      },
      [0, 0, 0, 0],
    );
