...
const categories: Record<ChangelogCategory, { label: string; color: string }> = {
  feature: { label: "Feature", color: "text-emerald-600" },
  improvement: { label: "Improvement", color: "text-blue-600" },
  fix: { label: "Fix", color: "text-amber-600" },
  breaking: { label: "Breaking change", color: "text-red-600" },
};

const defaultCategory = { label: "Update", color: "text-neutral-500" };
  ...
  if (entries.length === 0) {
    return (
      <div className="py-20 text-center">
        <p className="text-sm text-neutral-400">No changelog entries yet.</p>
      </div>
    );
  }
...
function Entry({ entry, index }: { entry: ChangelogEntry; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="border-b border-neutral-100 py-14 first:pt-0 last:border-0"
    >
      {/* Metadata line */}
      <p className="text-[13px] tabular-nums text-neutral-400">
        {format(date, "MMM d, yyyy")}
        <span className="mx-2 text-neutral-200">&middot;</span>
        v{entry.version}
        <span className="mx-2 text-neutral-200">&middot;</span>
      ...
      {/* Title */}
      <h2 className="mt-3 font-display text-2xl tracking-tight text-neutral-900 sm:text-3xl">
        {entry.title}
      </h2>
      ...
      {/* Content */}
      <div className="mt-6 rounded-xl border border-neutral-100">
        <div
