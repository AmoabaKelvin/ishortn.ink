...
function getFormDefaults(qrData: QRCodeData, geoRules?: GeoRulesData): EditQRCodeFormValues {
  return {
    ...
    tags: qrData.link?.tags || [],
    utmParams: qrData.link?.utmParams ?? undefined,
    geoRules:
      geoRules?.map((rule) => ({
        type: rule.type,
...
type EditQRCodeBodyProps = {
  qr: QRCodeData;
  geoRules: GeoRulesData | undefined;
  onClose: () => void;
};

// Mounted only once the drawer is open and geo rules have resolved, so the form
// initialises from correct defaults instead of resetting itself in an effect.
function EditQRCodeBody({ qr, geoRules, onClose }: EditQRCodeBodyProps) {
  const [tags, setTags] = useState<string[]>(qr.link?.tags || []);
  ...
  const form = useForm<EditQRCodeFormValues>({
    ...
    defaultValues: getFormDefaults(qr, geoRules),
  });
                            ...
                            <div className="absolute z-10 mt-1 max-h-48 w-full overflow-auto rounded-md border border-neutral-200 dark:border-border bg-white dark:bg-card shadow-md">
                              {filteredTags.map((tag) => (
                                <button
                                  key={tag}
                                  type="button"
                                  className="block w-full cursor-pointer px-3 py-2 text-left text-[13px] text-neutral-600 dark:text-neutral-400 transition-colors hover:bg-neutral-50 dark:hover:bg-accent/50 hover:text-neutral-900"
                                  onMouseDown={(e) => {
                                    e.preventDefault();
                                    addTag(tag);
                                  }}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-[12px] text-neutral-400 dark:text-neutral-500">
                      Scans show a &quot;not live yet&quot; page until this time. Print early,
                      launch on schedule.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
...
export function EditQRCodeDrawer({ qr, open, onClose }: EditQRCodeDrawerProps) {
  const { data: geoRules } = api.geoRules.getByLinkId.useQuery(
    { linkId: qr.link?.id ?? 0 },
    { enabled: open && !!qr.link?.id },
  );
  const geoRulesReady = !qr.link?.id || geoRules !== undefined;
      ...
      {open && (
        <>
          ...
          >
            ...
            {geoRulesReady ? (
              <EditQRCodeBody key={qr.id} qr={qr} geoRules={geoRules} onClose={onClose} />
            ) : (
