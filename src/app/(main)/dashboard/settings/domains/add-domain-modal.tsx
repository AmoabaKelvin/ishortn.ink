  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">{isProUser ? "Add New Domain" : "Upgrade to Add Domain"}</Button>
      </DialogTrigger>
      <DialogContent className={cn("sm:max-w-[425px]", satoshi.className)}>
        <DialogHeader>
          <DialogTitle>Add domain</DialogTitle>
          <DialogDescription>Add a new custom domain for your account.</DialogDescription>
        </DialogHeader>
        {isProUser ? (
          <motion.div
          ...
          >
            ...
            <motion.div variants={itemVariants} className="mt-1 text-sm text-yellow-600">
              Tip: If there's already content on your domain, consider using a subdomain like
              "links.example.com".
            </motion.div>
            <motion.div variants={itemVariants} className="mt-2 text-sm text-gray-500">
              <h4 className="mb-1 font-semibold">Quick Guide:</h4>
              <ul className="list-disc space-y-1 pl-5">
                <li>Ensure you own or have permission to use this domain.</li>
                <li>You'll need to configure DNS settings after adding the domain.</li>
          ...
          >
            <p className="text-yellow-600">
              Custom domains are only available for pro users. Please upgrade your plan to add a
              custom domain.
            </p>
          </motion.div>
        )}
        <DialogFooter>
          {isProUser ? (
            ...
            <Button type="button" className="mt-3 w-full" onClick={handleUpgrade}>
              Upgrade to Pro
            </Button>
          )}
        </DialogFooter>
