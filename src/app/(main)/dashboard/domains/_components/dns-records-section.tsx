...
export function DNSRecordsSection({ verificationChallenges }: DNSRecordsSectionProps) {
  ...
  const isApexDomain = verificationChallenges.some(
    (challenge) => challenge.type === "CNAME" && challenge.domain === "@",
  );
        ...
        {isApexDomain && (
          <div className="rounded-lg border border-neutral-200 dark:border-border bg-neutral-50 dark:bg-accent/50 p-3">
            ...
            <p className="mt-0.5 text-[11px] text-neutral-400 dark:text-neutral-500">
              Apex domains need a DNS provider that supports ALIAS, ANAME, or flattened CNAME
              records — for example Cloudflare, Namecheap, Porkbun, or Route 53. Create the record
              above as an ALIAS or ANAME record pointing to the same value. Alternatively, use a
              subdomain like links.yourdomain.com.
            </p>
          </div>
        )}
